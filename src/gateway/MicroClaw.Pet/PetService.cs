...
public class PetService : MicroService
{
    ...
    /// </summary>
    public virtual Task<IPet?> CreateOrLoadAsync(IMicroSession session, CancellationToken ct = default)
    ...
    private async Task<MicroPet?> LoadContextAsync(IMicroSession session, CancellationToken ct = default)
    {
        ...
        return new MicroPet(_serviceProvider, session, petState, petConfig, emotion, initialState);
    }
