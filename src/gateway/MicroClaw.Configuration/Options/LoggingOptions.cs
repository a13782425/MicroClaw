...
public sealed class LoggingOptions : IMicroClawConfigTemplate
{
    ...
    [
        new LoggingSinkOptions { Name = "console", Args = new LoggingSinkArgsOptions { OutputTemplate = "[{Timestamp:HH:mm:ss} {Level:u3}] {Message:lj}{NewLine}{Exception}" } },
        new LoggingSinkOptions { Name = "file", Args = new LoggingSinkArgsOptions { Path = "logs/microclaw-.log", RollingInterval = "day", RetainedFileCountLimit = 7, OutputTemplate = "{Timestamp:yyyy-MM-dd HH:mm:ss.fff zzz} [{Level:u3}] {Message:lj}{NewLine}{Exception}" } }
    ];
...
public sealed class LoggingSinkArgsOptions
{
    ...
    public int RetainedFileCountLimit { get; set; } = 99;
