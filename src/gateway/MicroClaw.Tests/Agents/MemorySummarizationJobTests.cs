...
public sealed class MemorySummarizationJobTests : IDisposable
{
    ...
    public async Task RunSummarizationAsync_WritesMemoryForSessionWithMessages()
    {
        ...
        var sessionStore = CreateSessionService(sessionsDir);
        ...
        ((ISessionRepository)sessionStore).AddMessage(session.Id, new SessionMessage(
    ...
    {
        string configDir = _configDir.Path;
        var sessionStore = CreateSessionService(_tempDir.Path);
        var providerStore = new ProviderConfigStore();
        ...
        var session = sessionStore.CreateSession("空会话", testProvider.Id, ChannelType.Web);
