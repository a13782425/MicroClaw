...
public sealed class DreamingJobTests : IDisposable
{
    ...
    }    private SessionService CreateSessionService(string sessionsDir)
    {
        TestConfigFixture.EnsureInitialized();
        var hub = Substitute.For<IHubContext<GatewayHub>>();
        var clients = Substitute.For<IHubClients>();
        hub.Clients.Returns(clients);
        clients.All.Returns(Substitute.For<IClientProxy>());
        var agSt = new AgentStore();
        return new SessionService(agSt, hub, [], new WebSessionChannel(hub), sessionsDir);
    }
    ...
    public async Task RunDreamingAsync_DisabledAgent_IsSkipped()
    {
        ...
        var sessionStore = CreateSessionService(_sessionsDir.Path);
        ...
        var job = new DreamingJob(agentStore, sessionStore, providerStore, clientFactory,
    ...
    public async Task RunDreamingAsync_AgentSessionsWithNoDailyMemories_IsSkipped()
    {
        ...
        sessionStore.CreateSession("无记忆会话", provider.Id, ChannelType.Web, agentId: agentId);
    ...
    public async Task RunDreamingAsync_AgentSessionsWithDailyMemories_UpdatesAgentDnaMemory()
    {
        ...
        var session = sessionStore.CreateSession("有记忆会话", provider.Id, ChannelType.Web, agentId: agentId);
    ...
    public async Task RunDreamingAsync_NoEnabledProviderAvailable_IsSkipped()
    {
        ...
        // 创建关联 Session
        var session = sessionStore.CreateSession("会话", disabledProvider.Id, ChannelType.Web, agentId: agentId);
