...
using FeishuNetSdk;
using FeishuNetSdk.Ccm;
using FeishuNetSdk.Ccm.Dtos;
...
public static class FeishuDocTools
{
            ...
            AIFunctionFactory.Create(
                    ...
                    CancellationToken ct) =>
                {
                    try
                    {
                        ...
                        var docResponse = await api.GetDocxV1DocumentsByDocumentIdRawContentAsync(docToken);
                        string? content = docResponse.Data?.Content;
                        if (content is null)
                        {
                            return (object)new { success = false, error = docResponse.Msg ?? "读取飞书文档内容失败，请确认文档 Token 正确且机器人有访问权限。" };
                        }
            ...
            AIFunctionFactory.Create(
                    ...
                    CancellationToken ct = default) =>
                {
                    try
                    {
                        ...
                        var block = BuildSdkBlock(content, contentType, language);
                        var writeResponse = await api.PostDocxV1DocumentsByDocumentIdBlocksByBlockIdChildrenAsync(
                            docToken, docToken,
                            new PostDocxV1DocumentsByDocumentIdBlocksByBlockIdChildrenBodyDto { Childrens = [block] });
                        if (writeResponse.Code != 0)
                        {
                            logger.LogWarning("飞书文档写入 API 返回错误 docToken={DocToken}: {Msg}", docToken, writeResponse.Msg);
                            return (object)new { success = false, error = writeResponse.Msg ?? "追加内容到飞书文档失败，请确认文档 Token 正确且机器人有编辑权限。" };
                        }
    ...
    /// <summary>
    /// Construct a FeishuNetSdk <see cref="Block"/> for appending text or code to a document.
    /// </summary>
    private static Block BuildSdkBlock(string content, string contentType, string language)
    {
        var textElement = new TextElement
        {
            TextRun = new TextElement.TextElementTextRun { Content = content }
        };
        bool isCode = contentType.Equals("code", StringComparison.OrdinalIgnoreCase);
        var blockText = new BlockText
        {
            Elements = [textElement],
            Style = isCode ? new TextStyle { Language = GetLanguageCode(language), Wrap = false } : new TextStyle()
        };
        return isCode
            ? new Block { BlockType = 14, Code = blockText }
            : new Block { BlockType = 2, Text = blockText };
    }
