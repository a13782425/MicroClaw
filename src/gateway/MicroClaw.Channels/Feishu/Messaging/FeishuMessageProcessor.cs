...
internal sealed class FeishuMessageProcessor(ISessionService sessionService, ILogger<FeishuMessageProcessor> logger, IAgentMessageHandler? agentHandler = null, IChannelRetryQueue? retryQueue = null, FeishuRateLimiter? rateLimiter = null, FeishuTokenCache? tokenCache = null, FeishuChannelHealthStore? healthStore = null, FeishuChannelStatsService? statsService = null) : IFeishuRetryProcessor
{
    ...
    public async Task ProcessMessageAsync(string userText, string? senderId, string chatId, string messageId, ChannelEntity channel, FeishuChannelSettings settings, string chatType = "p2p", IReadOnlyList<string>? mentionedOpenIds = null, IFeishuTenantApi? tenantApi = null, string? rootId = null, CancellationToken ct = default)
    {
        ...
        {
            logger.LogInformation("[{TraceId}] 会话 {SessionId} 未批准，拒绝处理 channel={ChannelId}", traceId, session.Id, channel.Id);
        ...
        catch (Exception ex)
        {
            ...
            if (retryQueue is not null)
            {
                try
                {
                    await retryQueue.EnqueueAsync("feishu", channel.Id, session.Id, messageId, userText, ex.Message, ct);
                    logger.LogInformation("[{TraceId}] AI 失败已入队重试 messageId={MessageId}", traceId, messageId);
                    await ReplyMessageAsync(settings, messageId, "AI 暂时无法处理，已加入重试队列，稍后会自动重试。", tenantApi, traceId, ct, replyInThread: replyInThread);
    ...
    /// </summary>
    public static string? ExtractText(ImMessageReceiveV1EventBodyDto? body, IReadOnlyDictionary<string, string>? mentionMap)
    {
        if (body?.Message is null) return null;
        ...
        switch (messageType)
        {
            ...
            case "file":
            {
                ...
                if (content?.FileSize is > 0)
                {
                    ...
                    string sizeStr = sizeKb >= 1024 ? $"{sizeKb / 1024:F1}MB" : $"{sizeKb:F0}KB";
    ...
    /// </summary>
    private async Task<string?> AddReactionAsync(FeishuChannelSettings settings, string messageId, IFeishuTenantApi? tenantApi, string traceId, CancellationToken ct)
    {
    ...
    /// </summary>
    private async Task RemoveReactionAsync(FeishuChannelSettings settings, string messageId, string reactionId, IFeishuTenantApi? tenantApi, string traceId, CancellationToken ct)
    {
        ServiceProvider? sp = null;
        try
        {
    ...
    /// </summary>
    public Task SendRetryReplyAsync(string messageId, string text, FeishuChannelSettings settings, CancellationToken ct = default) => ReplyMessageAsync(settings, messageId, text, tenantApi: null, traceId: "retry", ct);
    ...
    /// </summary>
    private async Task<(string Name, string? JobTitle)?> GetSenderInfoAsync(string openId, FeishuChannelSettings settings, IFeishuTenantApi? tenantApi, string traceId, CancellationToken ct)
    {
        ServiceProvider? sp = null;
        try
        {
