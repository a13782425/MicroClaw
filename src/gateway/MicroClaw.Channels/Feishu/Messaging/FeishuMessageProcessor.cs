    ...
    FeishuChannelStatsService? statsService = null)
{
        ...
        IFeishuTenantApi? tenantApi = null, string? rootId = null, CancellationToken ct = default)
    {
        ...
        {
            (string Name, string? JobTitle)? senderInfo = await GetSenderInfoAsync(senderId, tenantApi, traceId, ct);
    ...
    /// </summary>
    public async Task SendRetryReplyAsync(string messageId, string text, FeishuChannelSettings settings, CancellationToken ct = default)
    {
        await using ServiceProvider sp = BuildTempServiceProvider(settings);
    ...
    /// </summary>
    private static ServiceProvider BuildTempServiceProvider(FeishuChannelSettings settings)
    {
