...
public sealed class ChannelService : IChannelService
{
    ...
    public ChannelService(IServiceProvider serviceProvider, ILoggerFactory loggerFactory)
    {
        // Build providers internally — no longer injected from DI
        var providers = new Dictionary<ChannelType, IChannelProvider>
        {
            [ChannelType.Web] = new WebChannelProvider(serviceProvider.GetRequiredService<IMicroHubService>()), [ChannelType.WeCom] = new WeComChannelProvider(), [ChannelType.WeChat] = new WeChatChannelProvider(), [ChannelType.Feishu] = new FeishuChannelProvider(this, serviceProvider, loggerFactory),
        };
    ...
    public IReadOnlyList<ChannelEntityConfig> All
    {
        get
        {
            ...
            try
            {
                return [.. MicroClawConfig.Get<ChannelOptions>().Channels.Select(WithResolvedEnvVars)];
            }
            finally
            {
                _lock.ExitReadLock();
    ...
    public IReadOnlyList<ChannelEntityConfig> GetConfigsByType(ChannelType type)
    {
        ...
        try
        {
            return [.. MicroClawConfig.Get<ChannelOptions>().Channels.Where(c => c.ChannelType == type).Select(WithResolvedEnvVars)];
        }
        finally
        {
            _lock.ExitReadLock();
    ...
    public void EnsureWebChannel()
    {
        ...
        {
            var opts = MicroClawConfig.Get<ChannelOptions>();
            if (opts.Channels.Any(c => c.Id == ChannelUtils.WebChannelId)) return;
    ...
    public bool TryGet(string channelId, out IChannel? channel)
    {
        ...
        CacheEntry entry = _cache.AddOrUpdate(channelId, _ => new CacheEntry(fingerprint, provider.Create(config)), (_, existing) => existing.Fingerprint == fingerprint ? existing : new CacheEntry(fingerprint, provider.Create(config)));
    ...
    {
        return GetConfigsByType(type).Select(config => GetRequired(config.Id)).ToList().AsReadOnly();
    }
    
    public IReadOnlyList<IChannelProvider> GetProviders() => _providers.Values.OrderBy(static provider => provider.Type).ToList().AsReadOnly();
    ...
    public static string MaskSettingsSecrets(string? settingsJson, ChannelType type)
    {
        ...
        if (type == ChannelType.Feishu)
        {
            ...
            return JsonSerializer.Serialize(settings with { AppSecret = MaskSecret(settings.AppSecret), EncryptKey = MaskSecret(settings.EncryptKey), VerificationToken = MaskSecret(settings.VerificationToken), });
        }
    ...
    private static string MergeSettings(string existingJson, string incomingJson, ChannelType type)
    {
        ...
        return JsonSerializer.Serialize(incoming with { AppSecret = appSecret, EncryptKey = encryptKey, VerificationToken = verificationToken, });
    }
