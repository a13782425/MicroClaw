...
public interface IChannelProvider
{
    ...
    IChannel Create(ChannelEntityConfig config);

    Task PublishAsync(ChannelEntityConfig config, ChannelMessage message, CancellationToken cancellationToken = default);

    /// <summary>
    /// 处理渠道 Webhook 回调。<paramref name="headers"/> 供渠道实现内部做签名验证等安全检查。
    /// </summary>
    Task<WebhookResult> HandleWebhookAsync(ChannelEntityConfig config, string body,
        IReadOnlyDictionary<string, string?>? headers = null, CancellationToken cancellationToken = default);

    Task<ChannelTestResult> TestConnectionAsync(ChannelEntityConfig config, CancellationToken cancellationToken = default);
    ...
    /// 默认实现返回基础 ok 状态。
    /// </summary>
    Task<ChannelDiagnostics> GetDiagnosticsAsync(ChannelEntityConfig config, CancellationToken cancellationToken = default)
    ...
    /// </summary>
    Task<string?> HandleSessionMessageAsync(ChannelEntityConfig config, SessionMessage message,
