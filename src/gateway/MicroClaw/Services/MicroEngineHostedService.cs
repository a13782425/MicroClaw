...
public sealed class MicroEngineHostedService(MicroEngine microEngine, ILogger<MicroEngineHostedService> logger) : BackgroundService
{
    ...
    {
        await _microEngine.StartEngine(cancellationToken);
    ...
    public override async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using CancellationTokenRegistration registration = cancellationToken.Register(static state => { ((CancellationTokenSource)state!).CancelAfter(StopTimeout); }, engineStopCts);
            
            if (cancellationToken.IsCancellationRequested)
                engineStopCts.CancelAfter(StopTimeout);
            
            CancellationToken engineStopToken = engineStopCts.Token;
            await _microEngine.StopEngine(engineStopToken);
        }
        catch (Exception ex)
        {
