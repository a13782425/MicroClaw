...
public static class AgentEndpoints
{
    ...
    public static IEndpointRouteBuilder MapAgentEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/agents", (MicroAgentService agentService) => Results.Ok(agentService.GetAllAgents().Select(ToDto))).WithTags("Agents");
        
        endpoints.MapGet("/agents/{id}", (string id, MicroAgentService agentService) =>
        {
            MicroAgent? agent = agentService.GetAgentById(id);
            return agent is null ? Results.NotFound() : Results.Ok(ToDto(agent));
        }).WithTags("Agents");
        
        endpoints.MapPost("/agents", async (AgentCreateRequest req, MicroAgentService agentService, AgentDnaService agentDna, IServiceProvider sp, CancellationToken ct) =>
        {
            if (string.IsNullOrWhiteSpace(req.Name))
                return Results.BadRequest(new { success = false, message = "Name is required.", errorCode = "BAD_REQUEST" });
            
            MicroAgent newAgent = MicroAgent.New(sp: sp, name: req.Name.Trim(), description: req.Description ?? string.Empty, isEnabled: req.IsEnabled, contextWindowMessages: req.ContextWindowMessages ?? 10);
            ...
            if (req.MonthlyBudgetUsd.HasValue)
                newAgent.MonthlyBudgetUsd = req.MonthlyBudgetUsd;
            try
            {
                MicroAgent created = await agentService.CreateAgentAsync(newAgent, ct);
                try
                {
        ...
        endpoints.MapPost("/agents/update", async (AgentUpdateRequest req, MicroAgentService agentService, CancellationToken ct) =>
        {
            ...
            if (req.RoutingStrategy is not null && !Enum.TryParse<ProviderRoutingStrategy>(req.RoutingStrategy, ignoreCase: true, out _))
                return Results.BadRequest(new { success = false, message = $"Unknown routing strategy '{req.RoutingStrategy}'.", errorCode = "BAD_REQUEST" });
            
            try
            {
                MicroAgent saved = await agentService.UpdateAgentAsync(req.Id, existing =>
                {
                    if (req.Name is not null && !string.IsNullOrWhiteSpace(req.Name.Trim()))
                        existing.Name = req.Name.Trim();
                    if (req.Description is not null && !string.IsNullOrWhiteSpace(req.Description.Trim()))
                    ...
                    if (req.HasContextWindowMessages)
                        existing.ContextWindowMessages = req.ContextWindowMessages;
                    if (req.HasAllowedSubAgentIds) existing.UpdateAllowedSubAgentIds(req.AllowedSubAgentIds);
                    if (req.RoutingStrategy is not null && Enum.TryParse<ProviderRoutingStrategy>(req.RoutingStrategy, ignoreCase: true, out var parsedStrategy))
                        existing.RoutingStrategy = parsedStrategy;
                    if (req.HasMonthlyBudgetUsd)
                        existing.MonthlyBudgetUsd = req.MonthlyBudgetUsd;
                }, ct);
        ...
        {
            MicroAgent? agent = agentService.GetAgentById(id);
            return agent is null ? Results.NotFound(new { success = false, message = $"Agent '{id}' not found.", errorCode = "NOT_FOUND" }) : Results.Ok(agent.DisabledMcpServerIds);
        }).WithTags("Agents");
        ...
        endpoints.MapPost("/agents/{id}/mcp-servers", async (string id, AgentMcpServersRequest req, MicroAgentService agentService, McpServerConfigStore mcpStore, CancellationToken ct) =>
        {
            ...
            if (invalidIds.Count > 0)
                return Results.BadRequest(new { success = false, message = $"MCP Server(s) not found: {string.Join(", ", invalidIds)}", errorCode = "MCP_SERVER_NOT_FOUND" });
            
            try
            {
                MicroAgent saved = await agentService.UpdateAgentAsync(id, existing => existing.UpdateDisabledMcpServerIds(mcpIds), ct);
                return Results.Ok(new { saved.Id });
            }
            catch (KeyNotFoundException)
            {
        ...
        {
            IReadOnlyList<ToolGroupConfig> configs = req.Select(r => new ToolGroupConfig(r.GroupId, r.IsEnabled, r.DisabledToolNames ?? [])).ToList().AsReadOnly();
            
            try
            {
                MicroAgent saved = await agentService.UpdateAgentAsync(id, agent => agent.UpdateToolGroupConfigs(configs), ct);
                return Results.Ok(new { saved.Id });
            }
            catch (KeyNotFoundException)
            {
        ...
        endpoints.MapPost("/agents/{id}/skills", async (string id, AgentBoundSkillsRequest req, MicroAgentService agentService, SkillStore skillStore, CancellationToken ct) =>
        {
            ...
            if (invalidIds.Count > 0)
                return Results.BadRequest(new { success = false, message = $"Skill(s) not found: {string.Join(", ", invalidIds)}", errorCode = "SKILL_NOT_FOUND" });
            
            try
            {
                MicroAgent saved = await agentService.UpdateAgentAsync(id, existing => existing.UpdateDisabledSkillIds(skillIds), ct);
                return Results.Ok(new { saved.Id });
            }
            catch (KeyNotFoundException)
            {
        ...
        endpoints.MapGet("/agents/{id}/sub-agents", (string id, MicroAgentService agentService) =>
        {
            ...
            IEnumerable<MicroAgent> candidates = agentService.GetAllAgents().Where(a => a.IsEnabled && a.Id != id && agent.CanCallSubAgent(a.Id));
        ...
        {
            if (agentService.GetAgentById(id) is null)
                return Results.NotFound(new { success = false, message = $"Agent '{id}' not found.", errorCode = "NOT_FOUND" });
    ...
    private static AgentResp ToDto(MicroAgent a) => new(a.Id, a.Name, a.Description, a.IsEnabled, a.IsDefault, a.CreatedAtUtc, a.ContextWindowMessages, a.RoutingStrategy.ToString(), a.MonthlyBudgetUsd, a.DisabledSkillIds, a.DisabledMcpServerIds, a.ToolGroupConfigs, a.AllowedSubAgentIds);
}
public sealed record AgentResp(string Id, string Name, string Description, bool IsEnabled, bool IsDefault, DateTimeOffset CreatedAtUtc, int? ContextWindowMessages, string RoutingStrategy, decimal? MonthlyBudgetUsd, IReadOnlyList<string> DisabledSkillIds, IReadOnlyList<string> DisabledMcpServerIds, IReadOnlyList<ToolGroupConfig> ToolGroupConfigs, IReadOnlyList<string>? AllowedSubAgentIds);
public sealed record AgentCreateRequest(string Name, string? Description = null, bool IsEnabled = true, IReadOnlyList<string>? DisabledSkillIds = null, IReadOnlyList<string>? DisabledMcpServerIds = null, int? ContextWindowMessages = null, IReadOnlyList<string>? AllowedSubAgentIds = null, decimal? MonthlyBudgetUsd = null);
public sealed record AgentUpdateRequest(string Id, string? Name = null, string? Description = null, bool? IsEnabled = null, IReadOnlyList<string>? DisabledSkillIds = null, IReadOnlyList<string>? DisabledMcpServerIds = null, int? ContextWindowMessages = null, bool HasContextWindowMessages = false, IReadOnlyList<string>? AllowedSubAgentIds = null, bool HasAllowedSubAgentIds = false, string? RoutingStrategy = null, decimal? MonthlyBudgetUsd = null, bool HasMonthlyBudgetUsd = false);
...
public sealed record ToolGroupConfigRequest(string GroupId, bool IsEnabled, IReadOnlyList<string>? DisabledToolNames = null);