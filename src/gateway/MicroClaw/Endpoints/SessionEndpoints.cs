...
public static class SessionEndpoints
{
    ...
    public static IEndpointRouteBuilder MapSessionEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("/sessions", (CreateSessionRequest req, ISessionService sessions, ProviderConfigStore providerStore, AgentStore agentStore, ChannelConfigStore channelStore, SessionDnaService sessionDna) =>
        {
            ...
            Session created = sessions.CreateSession(req.Title.Trim(), req.ProviderId, channel.ChannelType, channelId: channelId, agentId: agentId);
        ...
        endpoints.MapPost("/sessions/{id}/dna", (string id, SessionDnaUpdateRequest req, ISessionRepository repo, SessionDnaService sessionDna) =>
        {
            ...
            if (!SessionDnaService.IsAllowedFileName(req.FileName))
                return Results.BadRequest(new { success = false, message = $"'{req.FileName}' is not a valid Session " + 
                    $"DNA file. Allowed: USER.md, AGENTS.md (SOUL.md 已迁移至 Agent 级别)", errorCode = "INVALID_FILE_NAME" });
